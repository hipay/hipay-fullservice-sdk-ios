...
- (void)viewDidLoad {
  ...
  self.tableCards.rowHeight = UITableViewAutomaticDimension;
  self.tableCards.estimatedRowHeight = 55;
...
- (void)setupCards {
  ...
  self.selectedCardsObjects = [[HPFPaymentCardTokenDatabase
      paymentCardTokensForCurrency:self.paymentPageRequest.currency]
      mutableCopy];

  self.cardsTouchID = [HPFPaymentCardTokenDatabase
      paymentCardTokensTouchIDForCurrency:self.paymentPageRequest.currency];

  NSMutableArray *cards =
      [NSMutableArray arrayWithCapacity:[self.selectedCardsObjects count]];
...
- (void)viewWillAppear:(BOOL)animated {
  ...
  id<HPFPaymentProductViewControllerDelegate> paymentProductViewDelegate =
      (id<HPFPaymentProductViewControllerDelegate>)
...
- (void)checkTransactionError:(NSError *)transactionError {
  ...
  [[HPFTransactionRequestResponseManager sharedManager]
      ...
      withCompletionHandler:^(HPFTransactionErrorResult *result) {
        [self.delegate paymentProductViewController:nil
...
- (void)checkTouchID {
  ...
  if (index != -1 && self.cardsTouchID != nil &&
      self.cardsTouchID.count > index) {
      ...
      if ([self canEvaluatePolicy]) {
        ...
        UIAlertController *alertViewController = [UIAlertController
            alertControllerWithTitle:
                HPFLocalizedString(
                    @"HPF_CARD_STORED_TOUCHID_NOT_ACTIVATED_TITLE")
                             message:HPFLocalizedString(
                                         @"HPF_CARD_STORED_TOUCHID_NOT_"
                                         @"ACTIVATED_MESSAGE")
...
- (BOOL)canEvaluatePolicy {
  ...
  // test if we can evaluate the policy, this test will tell us if Touch ID is
...
- (void)evaluatePolicy {
  ...
  [self.tableCards reloadSections:[NSIndexSet indexSetWithIndex:1]
                 withRowAnimation:UITableViewRowAnimationNone];

  // Show the authentication UI with our reason string.
  [context evaluatePolicy:LAPolicyDeviceOwnerAuthenticationWithBiometrics
          localizedReason:HPFLocalizedString(@"HPF_CARD_STORED_TOUCHID_REASON")
                    reply:^(BOOL success, NSError *authenticationError) {
                      dispatch_async(dispatch_get_main_queue(), ^{
                        ...
                        if (success) {
                          ...
                          [self.tableCards
                                reloadSections:[NSIndexSet indexSetWithIndex:1]
                              withRowAnimation:UITableViewRowAnimationNone];
                        }
                      });
                    }];
...
- (void)submit {
  ...
  [productCode replaceOccurrencesOfString:@" "
                               withString:@"_"
                                  options:NSCaseInsensitiveSearch
                                    range:NSMakeRange(0, productCode.length)];
  ...
  orderRequest.paymentMethod = [HPFCardTokenPaymentMethodRequest
      cardTokenPaymentMethodRequestWithToken:paymentCardToken.token
                                         eci:HPFECISecureECommerce
                     authenticationIndicator:self.paymentPageRequest
...
- (NSString *)tableView:(UITableView *)tableView
  ...
  case 1: {
    ...
    return [NSString
        stringWithFormat:HPFLocalizedString(@"HPF_TOTAL_AMOUNT"),
                         [numberFormatter
                             stringFromNumber:self.paymentPageRequest.amount]];
  }
...
- (UIImage *)brandToImage:(NSString *)brand {
  ...
  if (brand != nil && brand.length > 0) {
    if ([brand caseInsensitiveCompare:HPFPaymentProductCodeMasterCard] ==
        NSOrderedSame) {
      return [UIImage imageNamed:@"ic_credit_card_mastercard.png"
                               inBundle:HPFPaymentScreenViewsBundle()
          compatibleWithTraitCollection:nil];

    } else if ([brand caseInsensitiveCompare:HPFPaymentProductCodeVisa] ==
               NSOrderedSame) {
      return [UIImage imageNamed:@"VISA"
                               inBundle:HPFPaymentScreenViewsBundle()
          compatibleWithTraitCollection:nil];

    } else if ([brand caseInsensitiveCompare:
                          HPFPaymentProductCodeAmericanExpress] ==
               NSOrderedSame) {
      return [UIImage imageNamed:@"ic_credit_card_amex.png"
                               inBundle:HPFPaymentScreenViewsBundle()
          compatibleWithTraitCollection:nil];

    } else if ([brand caseInsensitiveCompare:HPFPaymentProductCodeDiners] ==
               NSOrderedSame) {
      return [UIImage imageNamed:@"ic_credit_card_diners.png"
                               inBundle:HPFPaymentScreenViewsBundle()
          compatibleWithTraitCollection:nil];

    } else if ([brand caseInsensitiveCompare:HPFPaymentProductCodeCB] ==
               NSOrderedSame) {
      return [UIImage imageNamed:@"CB.png"
                               inBundle:HPFPaymentScreenViewsBundle()
          compatibleWithTraitCollection:nil];

    } else if ([brand caseInsensitiveCompare:HPFPaymentProductCodeBCMC] ==
               NSOrderedSame) {
      return [UIImage imageNamed:@"BCMC"
                               inBundle:HPFPaymentScreenViewsBundle()
          compatibleWithTraitCollection:nil];
    }
  }

  return [UIImage imageNamed:@"ic_credit_card.png"
                           inBundle:HPFPaymentScreenViewsBundle()
      compatibleWithTraitCollection:nil];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView
  ...
  if (self.selectedCardsObjects.count == 0) {
    UITableViewCell *cardCell =
        [tableView dequeueReusableCellWithIdentifier:@"AnotherPaymentCell"
                                        forIndexPath:indexPath];
    cardCell.selectionStyle = UITableViewCellSelectionStyleNone;
    cardCell.textLabel.text =
        HPFLocalizedString(@"HPF_CARD_STORED_ANOTHER_SELECTION");
    return cardCell;
  } else {
    switch (indexPath.section) {
    case 0: {
      HPFPaymentCardTableViewCell *cardCell =
          [tableView dequeueReusableCellWithIdentifier:@"CardCell"
                                          forIndexPath:indexPath];

      HPFPaymentCardToken *paymentCardToken =
          self.selectedCardsObjects[indexPath.row];

      // 1. Format PAN (Last 4 Digits)
      NSString *rawPan = [paymentCardToken pan];
      if (rawPan.length >= 4) {
        NSString *lastFour = [rawPan substringFromIndex:rawPan.length - 4];
        cardCell.panLabel.text =
            [NSString stringWithFormat:@"•••• %@", lastFour];
      } else {
        cardCell.panLabel.text = rawPan;
      }

      // 2. Format Expiration Date (MM/YY)
      // Note: Ensure HPFPaymentCardToken has cardExpiryMonth/Year properties.
      // If these are strings in your model, use intValue; if NSNumbers, use
      // intValue.
      if ([paymentCardToken respondsToSelector:@selector(cardExpiryMonth)] &&
          [paymentCardToken respondsToSelector:@selector(cardExpiryYear)]) {

        // Use Key-Value Coding or direct access depending on your model
        // visibility
        NSNumber *monthNum = [paymentCardToken valueForKey:@"cardExpiryMonth"];
        NSNumber *yearNum = [paymentCardToken valueForKey:@"cardExpiryYear"];

        int month = [monthNum intValue];
        int year = [yearNum intValue];

        // Convert 2026 -> 26
        int shortYear = year > 100 ? year % 100 : year;

        cardCell.expirationLabel.text =
            [NSString stringWithFormat:@"%02d/%02d", month, shortYear];
      } else {
        cardCell.expirationLabel.text = @"";
      }
      ...
      // 4. Card Brand Image (Priority: Domestic > Brand)
      NSString *brandToDisplay = paymentCardToken.brand;

      // If we have a preferred domestic network, use it
      if (paymentCardToken.domesticNetwork != nil &&
          paymentCardToken.domesticNetwork.length > 0) {
        // Check for CB
        if ([paymentCardToken.domesticNetwork
                caseInsensitiveCompare:HPFPaymentProductCodeCB] ==
            NSOrderedSame) {
          brandToDisplay = HPFPaymentProductCodeCB;
        }
        // Check for Bancontact (BCMC)
        else if ([paymentCardToken.domesticNetwork
                     caseInsensitiveCompare:HPFPaymentProductCodeBCMC] ==
                 NSOrderedSame) {
          brandToDisplay = HPFPaymentProductCodeBCMC;
        }
      }

      cardCell.cardImageView.image =
          [self brandToImage:[brandToDisplay
    ...
    case 1: {
      HPFPaymentButtonTableViewCell *payCell =
          [self.tableCards dequeueReusableCellWithIdentifier:@"PaymentButton"];
...
- (void)tableView:(UITableView *)tableView
  ...
  if (self.selectedCardsObjects.count > 0) {
    ...
    case 0: {
      ...
      if (index != -1) {
        [indexesPath addObject:[NSIndexPath indexPathForItem:index
                                                   inSection:0]];
      }

      [self.tableCards beginUpdates];
      [self.tableCards reloadRowsAtIndexPaths:indexesPath
